package com.tansun.easycare.modules.gen.util;
...
import com.tansun.easycare.core.mapper.JaxbMapper;
import com.tansun.easycare.core.utils.DateUtils;
import com.tansun.easycare.core.utils.FileUtils;
import com.tansun.easycare.core.utils.FreeMarkers;
import com.tansun.easycare.core.utils.StringUtils;
import com.tansun.easycare.modules.gen.entity.GenCategory;
import com.tansun.easycare.modules.gen.entity.GenConfig;
		...
		for (GenTableColumn column : genTable.getColumnList()){
			...
			if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
				...
				if(StringUtils.startsWithIgnoreCase(column.getJdbcType(), "DATE")){
					column.setDictType("yyyyMMdd");
				} else {
					column.setDictType("yyyyMMdd HH:mm:ss");
				}
				...
				// 如果是主键
				if(genTable.getPkList().contains(column.getName())){
					column.setJavaType("Long");
				} else {
					column.setJavaType("java.math.BigDecimal");
					column.setShowType("BigDecimel");
				}
			...
			// 如果是主键,设置为查询条件
			column.setIsQuery(genTable.getPkList().contains(column.getName())?"1":"0");
			
			// 如果是主键,设置为显示内容
			column.setIsList(genTable.getPkList().contains(column.getName())?"1":"0");
			...
			if (!StringUtils.equalsIgnoreCase(column.getName(), "id")
					...
					// 设置为不可编辑
					&& !StringUtils.equalsIgnoreCase(column.getName(), "reg_org_no")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "reg_data_status")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "reg_check_status")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "reg_send_status")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "reg_operate_status")
					){
				column.setIsEdit("1");
			}
			...
			// 设置特定类型和字段名
			// 设置业务机构过滤树
			if (StringUtils.startsWithIgnoreCase(column.getName(), "reg_org_no")) {
			...
			// 设置四个平台标识字段
			// 数据状态
			if (StringUtils.startsWithIgnoreCase(column.getName(), "reg_data_status")) {
				column.setShowType("select");
				column.setDictType("REG_DATA_STATUS");
				column.setIsQuery("1");
				column.setIsList("1");
			}
			// 校验状态
			else if (StringUtils.startsWithIgnoreCase(column.getName(), "reg_check_status")) {
				column.setShowType("select");
				column.setDictType("REG_CHECK_STATUS");
				column.setIsQuery("1");
				column.setIsList("1");
			}
			// 报送状态
			else if (StringUtils.startsWithIgnoreCase(column.getName(), "reg_send_status")) {
				column.setShowType("select");
				column.setDictType("REG_SEND_STATUS");
				column.setIsQuery("1");
				column.setIsList("1");
			}
			// 操作状态
			else if (StringUtils.startsWithIgnoreCase(column.getName(), "reg_operate_status")) {
				column.setShowType("select");
				column.setDictType("REG_OPERATE_STATUS");
			}
			
			// 用户
			else if (StringUtils.startsWithIgnoreCase(column.getName(), "user_id")){
