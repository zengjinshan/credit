var fxqIStReportstatusList = new function() {
	return {
		// 查询
		query : function() {
			var queryCondition = $("#searchForm").serialize();
			$.ajax({
				url : ctx + "/fxq/fxqIStReportstatus/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/fxq/fxqIStReportstatus/formAdd");
		},
		// 修改
		modify : function() {
			...
			var tableName = "FXQ_I_ST_REPORTSTATUS";  // 表名
			var operateType = "EDT";  // 操作类型
			var regDataStatus = checkDatas[0][5];  // 数据状态
		...
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出可疑报告状态接口表数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/fxq/fxqIStReportstatus/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/fxq/fxqIStReportstatus/formEdit?dataDt=" + dataDt+ "&custNo=" + custNo);
}
...
function returnDelete() {
	...
	var dataDt = checkData[0];
	var custNo = checkData[1];
	confirmx('确认要删除该可疑报告状态接口表成功吗？', ctx + "/fxq/fxqIStReportstatus/delete?dataDt=" + dataDt+ "&custNo=" + custNo);
}
