var fxqDubiousTransactionList = new function() {
	return {
		// 查询
		query : function() {
			var queryCondition = $("#searchForm").serialize();
			$.ajax({
				url : ctx + "/fxq/fxqDubiousTransaction/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/fxq/fxqDubiousTransaction/formAdd");
		},
		// 修改
		modify : function() {
		...
		formView : function(dataDt,custNo,custAcctNum,txId){
			...
			go(ctx + "/fxq/fxqDubiousTransaction/formView?" + encodePram(keyMap));
		},
		// 导出Excel
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出可疑案例交易列表数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/fxq/fxqDubiousTransaction/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/fxq/fxqDubiousTransaction/formEdit?" + encodePram(keyMap));
}
...
function returnDelete() {
	...
	confirmx('确认要删除该可疑案例交易列表吗？', ctx + "/fxq/fxqDubiousTransaction/delete?" + encodePram(keyMap));
}
