var fxqIStRelationList = new function() {
	return {
		// 查询
		query : function() {
			var queryCondition = $("#searchForm").serialize();
			$.ajax({
				url : ctx + "/fxq/fxqIStRelation/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/fxq/fxqIStRelation/formAdd");
		},
		// 修改
		modify : function() {
			...
			var tableName = "FXQ_I_ST_RELATION";  // 表名
		...
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出可疑关系接口表数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/fxq/fxqIStRelation/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/fxq/fxqIStRelation/formEdit?dataDt=" + dataDt+ "&reportId=" + reportId+ "&txId=" + txId);
}
...
function returnDelete() {
	...
	var reportId = checkData[1];
	var txId = checkData[2];
	confirmx('确认要删除该可疑关系接口表成功吗？', ctx + "/fxq/fxqIStRelation/delete?dataDt=" + dataDt+ "&reportId=" + reportId+ "&txId=" + txId);
}
