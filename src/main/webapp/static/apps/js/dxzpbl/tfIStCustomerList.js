var tfIStCustomerList = new function() {
	return {
		...
		query : function() {
			...
			$.ajax({
				url : ctx + "/dxzpbl/tfIStCustomer/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/dxzpbl/tfIStCustomer/formAdd");
		},
		// 修改
		modify : function() {
			...
			var regDataStatus = checkDatas[0][32];  // 数据状态
			var regCheckStatus = checkDatas[0][33];  // 校验状态
			var regSendStatus = checkDatas[0][34];  // 报送状态
			var regOperateStatus = checkDatas[0][35];  // 操作状态
		...
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出可疑客户补录校验数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/dxzpbl/tfIStCustomer/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/dxzpbl/tfIStCustomer/formEdit?dataDt=" + dataDt+ "&custNo=" + custNo);
}
...
function returnDelete() {
	...
	var dataDt = checkData[0];
	var custNo = checkData[1];
	confirmx('确认要删除该可疑客户补录校验吗？', ctx + "/dxzpbl/tfIStCustomer/delete?dataDt=" + dataDt+ "&custNo=" + custNo);
}
