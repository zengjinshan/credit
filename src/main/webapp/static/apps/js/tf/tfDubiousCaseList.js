var tfDubiousCaseList = new function() {
	return {
		// 查询
		query : function() {
			var queryCondition = $("#searchForm").serialize();
			$.ajax({
				url : ctx + "/tf/tfDubiousCase/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/tf/tfDubiousCase/formAdd");
		},
		// 修改
		modify : function() {
		...
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出电信可疑案例数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/tf/tfDubiousCase/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
//			returnExclude();
		...
		showTransaction : function() {
			...
			go(ctx + "/tf/tfDubiousTransaction/list?"+encodePram(map));
		},
		//预览
		viewXml:function(){
			var checkDataIds = getTableCheckedIds("contentTable");
			...
			$.jBox.open("iframe:"+ctx+"/tf/showBSxml?"+encodePram(map),"报文预览",850,$(top.document).height()-240,{buttons:{"关闭":true}});
		},
		//意见
		advise:function(){
			var checkDataIds = getTableCheckedIds("contentTable");
			...
			go(ctx + "/tf/tfDubiousCase/advise?"+encodePram(map));
		}
	}
};
...
function returnExclude(){
	...
	confirmx('确认要排除该电信诈骗可疑案例吗？', ctx + "/tf/tfDubiousCase/exclude?"+encodePram(map));
}
/**
 * 提交 --在判断后进行调用
 */
function returnSubmit(){
	...
	confirmx('确认要提交该电信诈骗可疑案例吗？', ctx + "/tf/tfDubiousCase/submit?"+encodePram(map));
}
...
function returnBack(){
	...
	confirmx('确认要退回该电信诈骗可疑案例吗？', ctx + "/tf/tfDubiousCase/back?"+encodePram(map));
}
/**
 * 通过 --在判断后进行调用
 */
function returnPass(){
	...
	confirmx('确认要通过该电信诈骗可疑案例吗？', ctx + "/tf/tfDubiousCase/pass?"+encodePram(map));
}
...
function returnEdit() {
	...
	var dataDt = checkData[0];
	var custNo = checkData[1];
	go(ctx + "/tf/tfDubiousCase/formEdit?dataDt=" + dataDt+ "&custNo=" + custNo);
}
...
function returnDelete() {
	...
	var dataDt = checkData[0];
	var custNo = checkData[1];
	confirmx('确认要删除该电信可疑案例吗？', ctx + "/tf/tfDubiousCase/delete?dataDt=" + dataDt+ "&custNo=" + custNo);
}
