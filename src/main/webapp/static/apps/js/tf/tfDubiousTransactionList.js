var tfDubiousTransactionList = new function() {
	return {
		// 查询
		query : function() {
			var queryCondition = $("#searchForm").serialize();
			$.ajax({
				url : ctx + "/tf/tfDubiousTransaction/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		// 新增
		add : function() {
			go(ctx + "/tf/tfDubiousTransaction/formAdd");
		},
		// 修改
		modify : function() {
		...
		formView : function(dataDt,custNo,custAcctNum,txId){
			...
			go(ctx + "/tf/tfDubiousTransaction/formView?" + encodePram(keyMap));
		},
		// 导出Excel
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出电信可疑案例交易列表数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/tf/tfDubiousTransaction/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/tf/tfDubiousTransaction/formEdit?" + encodePram(keyMap));
}
...
function returnDelete() {
	...
	confirmx('确认要删除该电信可疑案例交易列表吗？', ctx + "/tf/tfDubiousTransaction/delete?" + encodePram(keyMap));
}
