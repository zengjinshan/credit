var tfIStAcctList = new function() {
	return {
		...
		query : function() {
			...
			$.ajax({
				url : ctx + "/tf/tfIStAcct/setQueryCondition",
				data : queryCondition,
				async : false,
				success : function(){
					$("#searchForm").submit();
		...
		formView : function(dataDt,custNo,custAcctNum){
			...
			go(ctx + "/tf/tfIStAcct/formView?" + encodePram(keyMap));
		},
		// 导出Excel
		exportExcel : function() {
			top.$.jBox.confirm("确认要导出可疑账户补录校验数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", ctx + "/tf/tfIStAcct/exportExcel");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
...
function returnEdit() {
	...
	go(ctx + "/tf/tfIStAcct/formEdit?" + encodePram(keyMap));
}
...
function returnDelete() {
	...
	confirmx('确认要删除该可疑账户补录校验吗？', ctx + "/tf/tfIStAcct/delete?" + encodePram(keyMap));
}
