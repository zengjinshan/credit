...
<body>
	...
	<shiro:hasPermission name="sys:role:assign">
		<button id="assignButton" class="btn_public" type="submit"><i class="icon button_set"></i>分配角色</button>
	</shiro:hasPermission>
		<!-- <input id="assignButton" class="btn btn-primary" type="submit" value="分配角色"/> -->
		<script type="text/javascript">
			$("#assignButton").click(function(){
				top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${role.id}", "分配角色",850,$(top.document).height()-240,{
					buttons:{"确定分配":"ok", "清除已选":"clear", "关闭":true}, bottomText:"通过选择机构，然后为列出的人员分配角色。",submit:function(v, h, f){
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${userList}" var="user">
			<tr>
				...
				<td>${user.phone}</td>
				<td>${user.mobile}</td>
				<shiro:hasPermission name="sys:role:assign"><td>
